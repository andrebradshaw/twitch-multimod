...
async function createUserAllowedListHTML() {
  ...
  let cont = ele('div');
  a(cont, [
    ['style', `display: grid; grid-template-columns: 1fr 1fr; grid-gap: 8px; max-height: 80vh; overflow-y: auto;`]
  ])
...
function addAllowedInputToList(obj,speaker_type,ref_elm) {
  ...
  let del = ele('div');
  a(del,[['jdat',`${JSON.stringify(obj)}`],['style',`cursor: pointer;`]]);
